...
module MRuby
  class Command
    ...
    def _run(options, params={})
      puts command + ' ' + ( options % params ) if NotFoundCommands.key? @command && Rake.verbose
  ...
  class Command::Linker < Command
    ...
    def run(outfile, objfiles, _libraries=[], _library_paths=[], _flags=[], _flags_before_libraries=[], _flags_after_libraries=[])
      ...
      _run link_options, { :flags => all_flags(_library_paths, _flags),
                            :outfile => path(outfile) , :objs => path(objfiles).join(' '),
  ...
  class Command::Archiver < Command
    ...
    def run(outfile, objfiles)
      ...
      puts "Command::Archiver run convert_mode=#{@convert_mode}" if Rake.verbose
      _run archive_options, { :outfile => path(outfile), :objs => path(objfiles).join(' ') }
    end
  end
  ...
  class Command::Git < Command
    ...
    def run_clone(dir, url, _flags = [])
      ...
      _run clone_options, { :flags => [flags, _flags].flatten.join(' '), :url => url, :dir => path(dir) }
    end
