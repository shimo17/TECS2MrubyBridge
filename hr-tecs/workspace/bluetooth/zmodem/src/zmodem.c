 ...
 * check the dteteata stream for 5 consecutive CAN characters;
...
int zmodem_recv_raw(zmodem_t* zm)
{
	int c;
	unsigned attempt;
    //syslog(LOG_EMERG,"zmodem_recv_raw");
...
/*			return(TIMEOUT);	removed June-12-2005 */
    ...
    //syslog(LOG_EMERG,"zmodem_recv_raw_yamamoto%d",c);
	return c;
}
...
int zmodem_recv_header_raw(zmodem_t* zm, int errors)
{
	...
	do {
		do {
			...
			if(is_cancelled(zm))
				return(ZCAN);
			//syslog(LOG_EMERG,"while_yama1");
		} while(c != ZPAD);
		//syslog(LOG_EMERG,"while_yama2");
