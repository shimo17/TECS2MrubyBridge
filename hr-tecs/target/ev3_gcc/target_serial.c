...
SIOPCB *sio_opn_por(ID siopid, intptr_t exinf) {
    ...
    if (p_siopcb->openflag){
        //syslog(LOG_EMERG,"opn_por1");
        return (p_siopcb);
    }
    //syslog(LOG_EMERG,"opn_por2");
    p_siopcb->opn_por((intptr_t)NULL);
    //syslog(LOG_EMERG,"opn_por_yama");
    p_siopcb->exinf = exinf;
    p_siopcb->openflag = true;
    //syslog(LOG_EMERG,"opn_por3");
    return (p_siopcb);
}
...
void bt_fetch_snd_buf(uint8_t **buf, uint32_t *bytes) {
    ...
    //syslog(LOG_EMERG,"bt_fetch_snd_bufid_yama%d",bt_send_buffer_idx);
