...
{
	//syslog(LOG_EMERG,"serial_rea_dat_yama");
	SPCB	*p_spcb;
	bool_t	buffer_empty;
	uint_t	reacnt = 0U;
	...
	if (sns_dpn()) {
	    //syslog(LOG_EMERG,"serial_rea_dat_yama1");				/* コンテキストのチェック */
		return(E_CTX);
	}
	if (!(1 <= portid && portid <= TNUM_PORT)) {
		//syslog(LOG_EMERG,"serial_rea_dat_yama2");
		return(E_ID);				/* ポート番号のチェック */
	}
	if (!PROBE_MEM_WRITE_SIZE(buf, len)) {
		//syslog(LOG_EMERG,"serial_rea_dat_yama3");
		return(E_MACV);				/* メモリアクセス権のチェック */
	}

	p_spcb = get_spcb(portid);
	//syslog(LOG_EMERG,"serial_rea_dat_yama4");
	if (!(p_spcb->openflag)) {
	    //syslog(LOG_EMERG,"serial_rea_dat_yama5");		/* オープン済みかのチェック */
		return(E_OBJ);
	}
	if (p_spcb->errorflag) {
	    //syslog(LOG_EMERG,"serial_rea_dat_yama6");		/* エラー状態かのチェック */
		return(E_SYS);
	}

	buffer_empty = true;
	//syslog(LOG_EMERG,"serial_rea_dat_yama7");			/* ループの1回めはwai_semする */
	while (reacnt < len) {
		if (buffer_empty) {
			//syslog(LOG_EMERG,"serial_rea_dat_yama8");
			SVC(rercd = wai_sem(p_spcb->p_spinib->rcv_semid),
										gen_ercd_wait(rercd, p_spcb));
			//syslog(LOG_EMERG,"serial_rea_dat_yama9");
			/* 上のwai_semからE_RLWAI，E_DLTが返った時は，セマフォを
				獲得していないため，何もせずにリターンしてよい．*/
		}
		//syslog(LOG_EMERG,"serial_rea_dat_yama10");
		SVC(rercd = serial_rea_chr(p_spcb, &c), rercd);
		*buf++ = c;
		reacnt++;
		buffer_empty = (bool_t) rercd;
		//syslog(LOG_EMERG,"serial_rea_dat_yama11");

		/*
		 *  エコーバック処理．
		 */
		if ((p_spcb->ioctl & IOCTL_ECHO) != 0U) {
			//syslog(LOG_EMERG,"serial_rea_dat_yama12");
			SVC(rercd = wai_sem(p_spcb->p_spinib->snd_semid),
										gen_ercd_wait(rercd, p_spcb));
			//syslog(LOG_EMERG,"serial_rea_dat_yama13");
			/* 上のwai_semからE_RLWAI，E_DLTが返った時は，セマフォを
				獲得していないため，何もせずにリターンしてよい．*/
			SVC(rercd = serial_wri_chr(p_spcb, c), rercd);
			//syslog(LOG_EMERG,"serial_rea_dat_yama14");
			/* 上のserial_wri_chrからE_RLWAI，E_DLTが返った時は，CRを送
				信して，LFを送る前にバッファフルになった場合である．バッ
				ファフルであり，セマフォも獲得していないため，何もせず
				にリターンしてよい．*/
			if (!((bool_t) rercd)) {
				//syslog(LOG_EMERG,"serial_rea_dat_yama15");
				SVC(sig_sem(p_spcb->p_spinib->snd_semid),
										gen_ercd_sys(p_spcb));
				//syslog(LOG_EMERG,"serial_rea_dat_yama16");
			}
		}
	}
	//syslog(LOG_EMERG,"serial_rea_dat_yama17");
	if (!buffer_empty) {
		//syslog(LOG_EMERG,"serial_rea_dat_yama18");
		SVC(sig_sem(p_spcb->p_spinib->rcv_semid), gen_ercd_sys(p_spcb));
		//syslog(LOG_EMERG,"serial_rea_dat_yama19");
	}
	ercd = E_OK;
	//syslog(LOG_EMERG,"serial_rea_dat_yama20");

  error_exit:
  //  syslog(LOG_EMERG,"serial_rea_dat_yama21");
...
{
	SPCB	*p_spcb;
    //syslog(LOG_EMERG,"portid%p",portid);
	if (sns_dpn()) {				/* コンテキストのチェック */
		return(E_CTX);
	}
	if (!(1 <= portid && portid <= TNUM_PORT)) {
	...
	p_spcb = get_spcb(portid);
	//syslog(LOG_EMERG,"get_spcb%p",p_spcb);
	if (!(p_spcb->openflag)) {		/* オープン済みかのチェック */
		return(E_OBJ);
	}
	if (p_spcb->errorflag) {		/* エラー状態かのチェック */
