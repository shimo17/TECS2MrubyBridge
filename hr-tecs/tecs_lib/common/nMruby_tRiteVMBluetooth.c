...
#include <mruby/dump.h>     //<--付け足し
#include <mruby/irep.h>     //<--付け足し
...
eMrubyBody_main(CELLIDX idx)
{
    ...
    while(1){
        //syslog(LOG_EMERG,"===MrubyBody_main===");
        flg_malloc = false;
        ...
        if(is_cSemaphore_joined()){
            ...
            flg_malloc = true;
            syslog(LOG_EMERG,"check1");
        if(is_cSemaphore_joined()){
            cSemaphore_signal();
        }
        syslog(LOG_EMERG,"check2");
        //syslog(LOG_EMERG,"Bluetooth Load complete");

        /* new interpreter instance */
        VAR_mrb = mrb_open();
        syslog(LOG_EMERG,"check3");
        if (VAR_mrb == NULL) {
...
//TODO: ZMODEMのコンポーネント化したら変更
    syslog(LOG_EMERG,"eRiteVM_reset");
...
mrb_init_mrb(CELLCB	*p_cellcb, mrb_state *mrb, mrbc_context *c){
    ...
    //mrb_irep *irep = mrb_read_irep(mrb, app_binary_buf);
    //mrb_run(mrb, mrb_proc_new(mrb, irep), mrb_top_self(mrb));

    /*-２つの mruby bytecode を動作させる-*/
    mrb_load_irep_cxt( mrb, ATTR_irepLib, c);           //もともとある方
    mrb_load_irep_cxt( mrb, VAR_irepApp, c);      //Bluetooth で送った方

    if (mrb->exc) {
        syslog(LOG_NOTICE,"-----mrb_init_mrb exit-----");
...
void test_serial_loader(CELLCB *p_cellcb, intptr_t portid) {
    ...
    syslog(LOG_NOTICE, "Start to receive an application file using ZMODEM protocol kyoto_gunma.");
    ...
    syslog(LOG_EMERG, "kakuninn_yamamoto%d.", ercd);
    if (ercd != E_OK) {
